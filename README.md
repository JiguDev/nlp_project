...
This project implements a tri-lingual (English, Hindi, Gujarati) government-service chatbot with a transformer model built from scratch in PyTorch, a shared SentencePiece tokenizer, and a lightweight FAISS-based retrieval layer for up-to-date official information.

## Features

- Input and output only in English (`en`), Hindi (`hi`), and Gujarati (`gu`)
- Transformer encoder-decoder implemented manually in PyTorch
- Shared SentencePiece tokenizer with language tokens `<en>`, `<hi>`, and `<gu>`
...
### Scalable Real-Data Ingestion (EN/HI/GU)
...
The default config is already wired to read these files if present:

- `data/multilingual/indiccorp/en.txt`
- `data/multilingual/indiccorp/hi-1.txt`
- `data/multilingual/indiccorp/gu.txt`

Only `en`, `hi`, and `gu` records are kept during ingestion.
...
The ingestion step creates filtered artifacts:
...
To train the tokenizer from the filtered corpus:
...
If you run pretraining directly, the script automatically prefers the filtered `data/processed/raw_corpus.jsonl` generated by ingestion.
...
If tokenizer/checkpoint artifacts are missing, the chatbot still runs in retrieval-grounded fallback mode and answers in the detected input language.
